...
# Función que define dx/dt para la bifurcación pitchfork supercrítica
dx_dt <- function(x, r) {
  r * x - x^3
}
...
for (r in r_vals) {
  ...
  bifurcation_data <- rbind(bifurcation_data,
                            data.frame(r = r, x = 0, stability = ifelse(r < 0, "Estable", "Inestable")))
}

# Definir la interfaz de usuario
ui <- fluidPage(
  theme = bs_theme(bootswatch = "minty"),
  titlePanel("Bifurcación Pitchfork Supercrítica"),
  sidebarLayout(
    sidebarPanel(
...
server <- function(input, output) {
  output$phasePlot <- renderPlot({
    ...
    equilibria <- if (input$r > 0) c(-sqrt(input$r), 0, sqrt(input$r)) else c(0)
